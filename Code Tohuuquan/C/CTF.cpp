...
void Cal(BigInt &nume, BigInt &deno) {
    BigInt T = GCD(nume, deno);
    nume = nume / T;
    deno = deno / T;
}
...
int main() {
    scanf("%d", &nTests); getline(cin, st);
    while (nTests--) {
        getline(cin, st);

        string sx, sy, sz;
        int i;
        for (i = 0; st[i] != '.'; i++) sx += st[i];
        for (++i; st[i] != '('; i++) sy += st[i];
        for (++i; st[i] != ')'; i++) sz += st[i];
        ...
        BigInt nume, deno;
        deno = Init(string(sz.size(), '9') + string(sy.size(), '0'));
        nume = x * deno + y * Init(string(sz.size(), '9')) + z;
