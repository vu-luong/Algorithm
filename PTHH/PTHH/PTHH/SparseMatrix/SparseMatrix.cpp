...
//	}
    ...
    int first = this->rows->at(row - 1) - 2;
    int last = this->rows->at(row) - 1;
    
    
    while (first + 1 < last) {
        int mid = (first + last) / 2;
        if (this->cols->at(mid) <= col) {
            first = mid;
        } else {
            last = mid;
        }
    ...
    if ((first != this->rows->at(row - 1) - 2) && this->cols->at(first) == col) {
        return this->vals->at(first);
    }

	return 0;
}

template<typename T> void SparseMatrix<T>::dispList() {
    
//    cout << "rows = " << this->rows->size() << endl;
//    for (int i = 0; i < this->rows->size(); i++) {
//        cout << i << " --- " << this->rows->at(i) << endl;
//    }
    
    int row = 1;
    
    for (int i = 0; i < this->vals->size(); i++) {
        if (i + 1 == this->rows->at(row)) {
            row++;
        }
        
        cout << vals->at(i) << " -> (" << row << ", " << this->cols->at(i) << ")\n";
    }
...
//	int actual = -1;
//
//    cout << pos << " " << this->rows->at(row) - 1 << endl;
//    
...
//	}
    ...
    if (last != this->rows->at(row) - 1) {
        pos = last;
        actual = this->cols->at(pos);
    } else {
        
        if (this->rows->at(row - 1) - 1 >= this->rows->at(row) - 1) {
            actual = -1;
        } else {
            actual = this->cols->at(this->rows->at(row) - 2);
            pos = this->rows->at(row) - 1;
        }
    }
    
    
//    cout << actual << " " << pos << " " << col << endl;
...
//	}
    ...
    for (int i = 0; i < this->vals->size(); i++) {
        ...
        int col = this->cols->at(i);
//        cout << vals->at(i) << " -> (" << row << ", " << this->cols->at(i) << ")\n";
        
        result[row - 1] += this->vals->at(i) * x[col - 1];
    }
